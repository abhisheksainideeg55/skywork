...
export default function ShiftPagination({
  currentPage = 1,
  totalPages = 1,
  totalItems = 0,
  pageSize = 10,
    ...
    <div className="flex flex-col sm:flex-row items-center justify-between gap-4 px-4 py-3 bg-white border-t border-slate-200/80 rounded-b-2xl">
      <div className="flex items-center gap-3 text-xs text-slate-500 font-medium">
        <span>
          ...
          <strong className="text-slate-800 font-semibold">{totalItems}</strong> shifts
        </span>

        {onPageSizeChange && (
          <div className="flex items-center gap-1.5 ml-2">
            <label htmlFor="shift-page-size" className="text-slate-400 text-xs">
              Rows:
            </label>
            <select
              id="shift-page-size"
      ...
      <div className="flex items-center gap-1.5">
        ...
        <div className="flex items-center gap-1">
          {Array.from({ length: totalPages }, (_, i) => i + 1)
            .filter((p) => {
              // keep first, last, and window around current page
              return (
                p === 1 ||
                p === totalPages ||
