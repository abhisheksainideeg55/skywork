...
export default function BreakToast({ toast, onClose }) {
  ...
  if (type === "error") {
    ...
    bgClass = "bg-indigo-950 text-indigo-100 border-indigo-800 shadow-xl shadow-indigo-950/30";
    icon = <FiInfo className="w-5 h-5 text-indigo-400 shrink-0" />;
  }
    ...
    <div className="fixed bottom-5 right-5 z-50 animate-bounce-in max-w-md">
      ...
      >
        ...
        <button
          type="button"
          onClick={onClose}
          className="ml-auto p-1 rounded-lg text-slate-400 hover:text-white hover:bg-white/10 transition-colors cursor-pointer"
