...
export default function EditBreakPolicyModal({
  isOpen,
  onClose,
  onUpdate,
  policy,
  ...
  useEffect(() => {
    if (policy) {
      setName(policy.name || "");
      setType(policy.type || "Lunch Break");
      setShiftType(policy.shiftType || "Day Shift");
      setStartTime(policy.startTime || "13:00");
      setEndTime(policy.endTime || "14:00");
      setDurationMinutes(policy.durationMinutes || 60);
      setLocation(policy.location || "");
      setAllowance(policy.allowance || "");
      setAlarmSound(policy.alarmSound || "chime");
      setIsMandatory(Boolean(policy.isMandatory));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await onUpdate(policy.id || policy.policyId, payload, currentUser);
      if (res?.success) {
        onClose();
      } else if (res?.error) {
        setError(res.error);
      ...
      setError(err.message || "Failed to update break policy");
    } finally {
      ...
      <div className="bg-white rounded-3xl w-full max-w-xl border border-slate-200 shadow-2xl overflow-hidden my-auto max-h-[92vh] flex flex-col animate-scale-up">
        ...
        <div className="bg-gradient-to-r from-amber-600 to-orange-600 p-4 sm:p-5 text-white flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base sm:text-lg font-bold">Edit Break Policy</h3>
              <p className="text-xs text-amber-100">
                Updating schedule for {policy.name} ({policy.shiftType})
              </p>
            </div>
          </div>
          <button
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              ...
              <select
                ...
                className="w-full px-3.5 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs sm:text-sm font-semibold text-slate-800 focus:outline-none focus:ring-2 focus:ring-amber-500 cursor-pointer"
              >
                {BREAK_TYPES.map((bt) => (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                className="w-full px-3.5 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs sm:text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-amber-500"
              />
            </div>
