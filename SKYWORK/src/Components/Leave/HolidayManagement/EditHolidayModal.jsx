...
export default function EditHolidayModal({
  ...
  const { holidays, updateHoliday } = useHoliday();
  ...
  // Initialize form when holiday changes or opened
  useEffect(() => {
    if (isOpen && holiday) {
      setName(holiday.name || "");
      setDate(holiday.date || "");
      setType(holiday.type || "National Holiday");
      setDuration(holiday.duration || "Full Day");
      setDescription(holiday.description || "");
      setStatus(holiday.status || "Active");
      setErrorMsg("");
      setSuccessMsg("");
      setDuplicateWarning("");
    }
  ...
  // Check duplicate date on date change excluding the current holiday id
  useEffect(() => {
    if (holiday && date && isDuplicateHolidayDate(holidays, date, holiday.id)) {
      const existing = holidays.find(
        (h) => h.date === date && h.id !== holiday.id
      );
      setDuplicateWarning(
  ...
  const handleSubmit = async (e) => {
    ...
    if (isDuplicateHolidayDate(holidays, date, holiday.id || holiday.holidayId)) {
      setErrorMsg(
        "A holiday already exists for this date. Please pick a different date."
      );
      return;
    }
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="edit-holiday-title"
    >
        ...
        <div className="flex items-center justify-between pb-4 border-b border-slate-100">
          <div>
            <span className="text-[11px] font-bold uppercase tracking-wider text-amber-600">
              Edit Holiday • {holiday.id}
            </span>
            <h2 id="edit-holiday-title" className="text-xl font-bold text-slate-900">
              Edit Holiday
            </h2>
          </div>
          <button
        ...
        <form onSubmit={handleSubmit} className="mt-5 space-y-4">
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Provide background or observance details..."
