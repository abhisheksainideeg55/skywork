        ...
        <table className="w-full text-left border-collapse min-w-[1050px]">
          <thead>
            <tr className="bg-slate-50/90 border-b border-slate-200/80 text-[11px] font-bold uppercase tracking-wider text-slate-500">
              ...
              <th className="py-3.5 px-4 min-w-[120px]">Leave Type</th>
              ...
              <th className="py-3.5 px-4 min-w-[160px]">Reason</th>
                ...
                >
                  <div className="flex flex-col items-center justify-center gap-2">
                    ...
                    <p className="text-xs text-slate-400">
                      Try adjusting your search criteria or apply for a new leave.
                    </p>
                  </div>
                </td>
              </tr>
                      ...
                      <div className="flex items-center justify-center gap-1.5">
                        ...
                        {isHRView && !canActOnRecord && (
                          <span 
                            ...
                            title="HR Leave & Half-day applications can only be Approved, Rejected, or Updated by Super Admin."
                          >
                        ...
                        {/* 3. HR / Super Admin Controls for Pending Leaves */}
                        {isHRView && canActOnRecord && isPending && (
                          <>
                            <button
                              ...
                              title={isSuperAdmin ? "Super Admin Approve Leave" : "Approve Leave"}
                            >
                            ...
                            <button
                              ...
                              title={isSuperAdmin ? "Super Admin Reject Leave" : "Reject Leave"}
                            >
                        ...
                        {/* 4. HR / Super Admin Controls for Approved Leaves (Can Reject or Reset to Pending) */}
                        {isHRView && canActOnRecord && isApproved && (
                          <>
                            <button
                              ...
                              onClick={() => onRejectRecord && onRejectRecord(item)}
                              className="p-1.5 rounded-lg text-rose-600 hover:bg-rose-50 border border-rose-200 transition-colors cursor-pointer"
                              title="Change Status: Reject this Approved Leave"
                            >
                              <FiX className="w-4 h-4" />
                            </button>
                            {onResetPendingRecord && (
                        ...
                        {/* 5. HR / Super Admin Controls for Rejected Leaves (Can Approve or Reset to Pending) */}
                        {isHRView && canActOnRecord && isRejected && (
                          <>
                            <button
                              ...
                              onClick={() => onApproveRecord && onApproveRecord(item)}
                              className="p-1.5 rounded-lg text-emerald-600 hover:bg-emerald-50 border border-emerald-200 transition-colors cursor-pointer"
                              title="Change Status: Approve this Rejected Leave"
                            >
                              <FiCheck className="w-4 h-4" />
                            </button>
                            {onResetPendingRecord && (
                        ...
                        {/* 6. HR / Super Admin Controls for Cancelled Leaves (Can Re-Approve) */}
                        {isHRView && canActOnRecord && item.status === "Cancelled" && (
                          <button
                            type="button"
                            onClick={() => onApproveRecord && onApproveRecord(item)}
                            className="p-1.5 rounded-lg text-emerald-600 hover:bg-emerald-50 border border-emerald-200 transition-colors cursor-pointer"
                            title="Change Status: Approve this Cancelled Leave"
                          >
                            <FiCheck className="w-4 h-4" />
                          </button>
                        )}

                        {/* 7. User Cancel Request (Pending only) */}
