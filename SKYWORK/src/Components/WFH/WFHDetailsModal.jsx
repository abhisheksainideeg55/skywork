...
export default function WFHDetailsModal({
  isOpen,
  onClose,
  record,
  isHRView = false,
    ...
    >
      <div className="bg-white rounded-3xl max-w-md w-full p-6 sm:p-7 shadow-2xl border border-slate-200 animate-in fade-in zoom-in-95 duration-200 my-8">
        ...
        <div className="flex items-center justify-between pb-4 border-b border-slate-100">
          <div>
            <span className="text-[11px] font-bold uppercase tracking-wider text-slate-400">
              WFH Record • {record.id}
            </span>
            <h2 id="wfh-details-title" className="text-xl font-bold text-slate-900">
              WFH Request Details
            </h2>
          </div>
          <button
        ...
        <div className="py-4 space-y-4">
          ...
          {/* Key WFH Parameters Grid */}
          <div className="grid grid-cols-2 gap-3 text-xs">
            <div className="p-3 rounded-xl bg-slate-50 border border-slate-100">
              <span className="text-slate-400 block mb-1">WFH Type</span>
              <span className="font-bold text-slate-800">{record.wfhType}</span>
            </div>
            <div className="p-3 rounded-xl bg-slate-50 border border-slate-100">
          ...
          {record.contactNumber && (
            <div className="p-3 rounded-xl bg-slate-50 border border-slate-100 flex items-center justify-between text-xs">
              <div className="flex items-center gap-2 text-slate-600 font-medium">
                <FiPhone className="w-3.5 h-3.5 text-indigo-600" />
          ...
          {record.deliverables && (
            <div className="p-3.5 rounded-xl bg-indigo-50/50 border border-indigo-100 text-xs">
              <span className="text-indigo-900 font-bold flex items-center gap-1.5 mb-1">
                <FiCheckSquare className="w-3.5 h-3.5 text-indigo-600" />
                Planned Deliverables & Tasks
              </span>
              <p className="text-indigo-800 leading-relaxed">{record.deliverables}</p>
            </div>
          )}
          ...
          <div className="p-3.5 rounded-xl bg-slate-50 border border-slate-100 text-xs space-y-2">
            <div className="flex items-center justify-between">
              ...
              <WFHStatusBadge status={record.status} />
            </div>
            ...
            <>
              {!canActOnRecord ? (
                <div className="p-3.5 rounded-2xl bg-purple-50 border border-purple-200/80 flex items-center gap-2.5 text-xs text-purple-900 font-medium">
                  ...
                  <span>HR WFH applications can only be approved, rejected, or updated by Super Admin.</span>
                </div>
              ) : (
                <div className="p-3.5 rounded-2xl bg-indigo-50/50 border border-indigo-100/80 space-y-2.5">
                  ...
                  <div className="grid grid-cols-3 gap-2 pt-1">
                    ...
                    <button
                      ...
                      title={isApproved ? "Already Approved" : "Approve WFH Request"}
                    >
                    ...
                    <button
                      ...
                      title={isRejected ? "Already Rejected" : "Reject WFH Request"}
                    >
