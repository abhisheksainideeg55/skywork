...
export default function WFHRejectModal({
  ...
  const handleSubmit = (e) => {
    ...
    if (!reason || reason.trim().length < 3) {
      setError("Please provide a valid reason for rejecting this WFH request.");
      return;
    }
    setError("");
        ...
        <div className="flex items-center justify-between pb-3 border-b border-slate-100">
          <div className="flex items-center gap-2 text-rose-600">
            ...
            <h3 className="text-base font-bold text-slate-900">
              {isPreviouslyApproved ? "Reject Approved WFH" : "Reject WFH Request"}
            </h3>
          </div>
          <button
            type="button"
        ...
        <form onSubmit={handleSubmit} className="pt-4 space-y-4">
          <div className="p-3 bg-rose-50/70 border border-rose-100 rounded-xl text-xs text-rose-900">
            <p className="leading-relaxed">
              {isPreviouslyApproved ? (
                <>
                  You are modifying an <strong className="font-semibold text-rose-950">Approved WFH Request</strong> for{" "}
                  <strong>{record.employeeName}</strong>. Changing status to <strong>Rejected</strong> will decline their remote work arrangement.
                </>
              ) : (
                <>
                  You are rejecting the <strong>{record.wfhType}</strong> ({record.totalDays} days) requested by{" "}
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="e.g. Critical in-person client presentation, on-site hardware testing required..."
