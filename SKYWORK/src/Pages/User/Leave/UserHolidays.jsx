...
export default function UserHolidays() {
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute(
      "download",
      `company_holidays_${new Date().toISOString().slice(0, 10)}.csv`
    );
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 text-xs font-semibold text-slate-400 uppercase tracking-wider">
            ...
            <span>•</span>
            <span className="text-indigo-600">Company Holidays</span>
          </div>
          <h1 className="text-2xl sm:text-3xl font-bold text-slate-900 tracking-tight mt-1">
            Company Holiday Calendar
          </h1>
          <p className="text-xs sm:text-sm text-slate-500 mt-0.5">
            View official company holidays, upcoming celebrations, and annual off-days.
          </p>
        </div>
